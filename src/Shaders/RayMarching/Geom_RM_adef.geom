...
void main()
{
    ...
    // T = [v0 v1 v2 v3] * [a b c d]^-1  (scene → unit prism space)
    mat4 pMat_T = mat4(verteces[0], verteces[1], verteces[2], verteces[3]);
    mat4 vMat   = mat4(uprismPos[0], uprismPos[1], uprismPos[2], uprismPos[3]);
    mat4 T      = vMat * inverse(pMat_T);

    // M = [a d e f] → [(u_a,0) (u_a,1) (u_b,1) (u_c,1)]  (scene → texture space)
    // Anchoring to all three top vertices gives exact h=1 at every primitive corner;
    // error shifts to h≈0 (bottom face), which is less visible on curved meshes.
    mat4 uMat = mat4(texPos[0], texPos[3], texPos[4], texPos[5]);
    mat4 pMat = mat4(verteces[0], verteces[3], verteces[4], verteces[5]);
    mat4 M    = uMat * inverse(pMat);
